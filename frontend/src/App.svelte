<script>
  ...
  let currentTab = "campaign"; // "campaign" or "ai"
...
<div class="min-h-screen flex flex-col font-body-md text-body-md bg-background text-on-surface">
  ...
  <header class="fixed top-0 w-full bg-surface z-50 flex justify-between items-center h-16 px-md border-b border-outline-variant">
    <div class="flex items-center gap-4">
      ...
      <div class="flex gap-2">
        <button
          on:click={() => currentTab = "campaign"}
          class="font-headline-md text-[16px] md:text-headline-md px-3 py-1 font-bold rounded-lg transition-all {currentTab === 'campaign' ? 'text-primary bg-primary/10' : 'text-on-surface-variant hover:bg-surface-container-low'}"
        >
          Campaign Setup
        </button>
        <button
          on:click={() => currentTab = "ai"}
          class="font-headline-md text-[16px] md:text-headline-md px-3 py-1 font-bold rounded-lg transition-all {currentTab === 'ai' ? 'text-primary bg-primary/10' : 'text-on-surface-variant hover:bg-surface-container-low'}"
        >
          AI Persona
        </button>
      </div>
  ...
  {:else}
    <main class="flex-grow pt-20 pb-12 w-full overflow-y-auto">
      <PromptConfig showHeader={false} />
    </main>
  {/if}
