<script>
  ...
  let selectedId = null;
  let replyText = '';
  let isInboxLoading = true;
  let inboxSearchQuery = '';
  let inboxFilter = 'All'; // 'All', 'Active', 'Waiting'
  let isInboxMobileView = false;
  let showMobileChat = false;

  function handleInboxResize() {
    isInboxMobileView = window.innerWidth < 768;
  }
  ...
  $: filteredConversations = conversations.filter(c => {
    ...
    if (inboxFilter === 'Active') return c.status === 'ACTIVE';
  ...
  onMount(() => {
    fetchConversations();
    handleInboxResize();
    window.addEventListener('resize', handleInboxResize);
    return () => window.removeEventListener('resize', handleInboxResize);
  });
</script>
...
<div class="min-h-screen flex flex-col md:flex-row font-body-md text-body-md bg-surface text-on-surface">
  ...
  <aside class="fixed inset-y-0 left-0 z-50 hidden md:flex flex-col py-6 h-full w-72 bg-surface border-r border-outline-variant">
    ...
    <nav class="flex-1 space-y-1">
      ...
      <button
        ...
        class="w-[calc(100%-16px)] flex items-center gap-3 rounded-full mx-2 px-4 py-3 text-left transition-all duration-300
          {activeTab === 'inbox' ? 'bg-primary text-on-primary font-bold' : 'text-on-surface-variant font-medium hover:bg-surface-container-high'}"
      >
        <span class="material-symbols-outlined" style="font-variation-settings: 'FILL' {activeTab === 'inbox' ? 1 : 0};">chat</span>
        <span class="font-label-md text-label-md">Conversations (Inbox)</span>
      </button>
  ...
  <main class="flex-grow pt-20 pb-28 md:pb-8 md:pl-72 w-full min-h-screen">
    <div class="px-margin-mobile md:px-margin-desktop max-w-6xl mx-auto w-full">
      ...
      <!-- ==================== VIEW 3: UNIFIED SVELTE INBOX DASHBOARD ==================== -->
      {#if activeTab === "inbox"}
        <section class="animate-fadeIn mt-4" aria-labelledby="inbox-heading">
          <div class="flex flex-col md:flex-row md:items-stretch gap-6 h-[calc(100vh-12rem)] bg-white border border-outline-variant rounded-2xl overflow-hidden shadow-sm">

            <!-- MASTER PANEL: Left Conversation List -->
            <div class="w-full md:w-[380px] md:border-r border-outline-variant flex flex-col bg-surface transition-all {isInboxMobileView && showMobileChat ? 'hidden' : 'flex'}">
              <!-- Search and Filters Section -->
              <section class="p-4 bg-surface sticky top-14 z-40 border-b border-outline-variant/30">
                <div class="relative flex items-center mb-4">
                  ...
                  <input
                    ...
                    class="w-full pl-10 pr-4 py-2 bg-surface-container-lowest border border-outline-variant rounded-lg font-medium text-xs focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all"
                  />
                </div>
                ...
                <div class="flex gap-2 overflow-x-auto no-scrollbar pb-1">
                  {#each ['All', 'Active', 'Waiting'] as filter}
                    <button
                      on:click={() => inboxFilter = filter}
                      class="px-4 py-1.5 rounded-full text-xs font-semibold transition-colors shrink-0 {inboxFilter === filter ? 'bg-primary text-on-primary' : 'bg-surface-container-high text-on-surface-variant hover:bg-surface-container-highest'}"
                    >
                      {filter}
                    </button>
                  {/each}
              ...
              {#if isInboxLoading && conversations.length === 0}
                  ...
                  {#if waitingConversations.length > 0}
                    ...
                    {#each waitingConversations as chat (chat.id)}
                      <button
                        ...
                        class="w-full text-left relative bg-surface-container-lowest border-y border-outline-variant/30 px-4 py-4 flex items-start gap-3 hover:bg-surface-container-low transition-colors duration-150 cursor-pointer {selectedId === chat.id ? 'bg-surface-container' : ''} focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
                      >
            ...
            <!-- DETAIL PANEL: Right Conversation details -->
            <div class="flex-grow flex flex-col bg-surface-container-low transition-all {isInboxMobileView && !showMobileChat ? 'hidden' : 'flex'}">
              {#if selectedChat}
                <!-- Detail Header -->
                <header class="h-16 px-4 border-b border-outline-variant bg-surface-container-lowest flex items-center justify-between z-10">
                  <div class="flex items-center gap-3">
                    {#if isInboxMobileView}
                ...
                <!-- Messages Area -->
                <div class="flex-grow overflow-y-auto p-4 space-y-4 bg-surface-container-low flex flex-col justify-end min-h-[300px]">
                  <div class="space-y-4">
  ...
  <nav class="fixed bottom-0 left-0 w-full z-40 flex justify-around items-center px-2 py-3 pb-safe bg-surface border-t border-outline-variant rounded-t-xl shadow-lg md:hidden">
    ...
    <button
      on:click={() => activeTab = "inbox"}
      class="flex flex-col items-center justify-center rounded-full px-4 py-1.5 transition-transform duration-150
        {activeTab === 'inbox' ? 'bg-primary/10 text-primary scale-105' : 'text-on-surface-variant'}"
      aria-label="Inbox View"
    >
      <span class="material-symbols-outlined" style="font-variation-settings: 'FILL' {activeTab === 'inbox' ? 1 : 0};">chat</span>
      <span class="font-label-sm text-[10px] font-semibold mt-0.5">Inbox</span>
    </button>
