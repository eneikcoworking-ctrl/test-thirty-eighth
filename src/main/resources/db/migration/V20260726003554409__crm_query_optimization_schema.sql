-- CRM Query Optimization Schema

CREATE TABLE IF NOT EXISTS crm_telegram_accounts (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    phone_number VARCHAR(50) NOT NULL,
    session_status VARCHAR(50) NOT NULL,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP NOT NULL
);

CREATE TABLE IF NOT EXISTS crm_leads (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    username VARCHAR(100),
    phone_number VARCHAR(50),
    status VARCHAR(50) NOT NULL,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP NOT NULL
);

CREATE TABLE IF NOT EXISTS crm_chats (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    telegram_account_id BIGINT NOT NULL,
    lead_id BIGINT NOT NULL,
    status VARCHAR(50) NOT NULL,
    last_message_at TIMESTAMP WITH TIME ZONE NOT NULL,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP NOT NULL,
    CONSTRAINT fk_crm_chats_account FOREIGN KEY (telegram_account_id) REFERENCES crm_telegram_accounts(id),
    CONSTRAINT fk_crm_chats_lead FOREIGN KEY (lead_id) REFERENCES crm_leads(id)
);

CREATE TABLE IF NOT EXISTS crm_messages (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    chat_id BIGINT NOT NULL,
    sender_type VARCHAR(50) NOT NULL,
    text TEXT NOT NULL,
    sent_at TIMESTAMP WITH TIME ZONE NOT NULL,
    CONSTRAINT fk_crm_messages_chat FOREIGN KEY (chat_id) REFERENCES crm_chats(id)
);

-- Index for fast unified inbox queries by status and account, ordered by last_message_at DESC
CREATE INDEX IF NOT EXISTS idx_crm_chats_account_status_last_msg ON crm_chats(telegram_account_id, status, last_message_at DESC);

-- View for unified inbox presentation
CREATE OR REPLACE VIEW crm_unified_inbox_view AS
SELECT
    c.id AS chat_id,
    c.status AS chat_status,
    c.last_message_at AS last_message_at,
    a.id AS telegram_account_id,
    a.phone_number AS account_phone_number,
    a.session_status AS account_status,
    l.id AS lead_id,
    l.username AS lead_username,
    l.phone_number AS lead_phone_number,
    l.status AS lead_status
FROM crm_chats c
JOIN crm_telegram_accounts a ON c.telegram_account_id = a.id
JOIN crm_leads l ON c.lead_id = l.id;
